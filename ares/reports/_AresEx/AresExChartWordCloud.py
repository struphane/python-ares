...
from ares.Lib.graph import AresHtmlGraphWordCloud
from ares.Lib import AresImports

def report(aresObj):
  aresObj.title("WordCloud objects")
  aresObj.paragraph("aresObj function signature")
  aresObj.preformat('''
  wordcloud(self, values, header, headerBox=None, cssCls=None, cssAttr=None, mockData=False)
  ''')

  aresObj.paragraph("Reference, Javascript and CSS needs")
  impManager = AresImports.ImportManager()
  cssImports = impManager.cssResolve(AresHtmlGraphWordCloud.WordCloud.reqCss)
  jsImports = impManager.jsResolve(AresHtmlGraphWordCloud.WordCloud.reqJs)
  aresObj.row([aresObj.paragraph(jsImports),
               aresObj.list(AresHtmlGraphWordCloud.WordCloud.references),
  ...
  p = aresObj.preformat('''
    chart = aresObj.wordcloud(data, header, headerBox='Currencies')
    chart.setKeys(['CCY'])
    chart.setVals(['VAL'])
  ''')
  chart = aresObj.wordcloud(data, header, headerBox='Currencies')
  ...
  next = aresObj.internalLink("Next", 'AresExChartHorizBar')
  prev = aresObj.internalLink("Previous", 'AresExChartSpider')
