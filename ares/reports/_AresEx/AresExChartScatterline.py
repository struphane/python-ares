...
from ares.Lib.graph import AresHtmlGraphScatterPlusLine
from ares.Lib import AresImports

def report(aresObj):
  aresObj.title("NvD3ScatterPlusLineChart objects")
  aresObj.paragraph("aresObj function signature")
  aresObj.preformat('''
  def scatterline(self, values, header, headerBox=None, cssCls=None, cssAttr=None, mockData=False)
  ''')

  aresObj.paragraph("Reference, Javascript and CSS needs")
  impManager = AresImports.ImportManager()
  cssImports = impManager.cssResolve(AresHtmlGraphScatterPlusLine.NvD3ScatterPlusLineChart.reqCss)
  jsImports = impManager.jsResolve(AresHtmlGraphScatterPlusLine.NvD3ScatterPlusLineChart.reqJs)
  aresObj.row([aresObj.paragraph(jsImports),
               aresObj.list(AresHtmlGraphScatterPlusLine.NvD3ScatterPlusLineChart.references),
  ...
  p = aresObj.preformat('''
    chart = aresObj.scatterline(data, header, headerBox='Currencies')
    chart.setSeries(['PRD'])
    chart.setY(['VAL'])
    chart.setX('PTF')
  ''')
  ...
  next = aresObj.internalLink("Next", 'AresExChartStackedArea')
  prev = aresObj.internalLink("Previous", 'AresExChartScatter')
