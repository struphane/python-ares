...
def getTeamData(team, sqlCon):
  """ """
  files = """ SELECT  file_map.alias as "file_code", file_map.disk_name as "file_name"
                      FROM file_auth
                      ...
                      GROUP BY "team", "file_code", "file_name" """ % team
...
def getFilesPermissions(sqlCon, files, team):
  ...
  fileDetails = """ SELECT file_map.disk_name, team_def.team_name, file_auth.temp_owner, file_auth.stt_dt, file_auth.end_dt
                  ...
                  WHERE file_map.disk_name in ('%s')
                  AND team_name != '%s' """ % ("','".join(files), team)

  cache = {}
  for rec in sqlCon.select(fileDetails):
    cache.setdefault(rec['file_map.disk_name', []]).append({'team': rec['team_def.team_name'], 'tmp_owner': rec['file_auth.temp_owner'], 'stt_dt': rec['file_auth.stt_dt'], 'end_dt': rec['file_auth.end_dt']})
...
def report(aresObj):
  ...
  fileCache = {}
  summaryRecSet = []
  for rec in getTeamData(aresObj.http['TEAM_DSC'], sqlCon):
    fileLst.append(rec['file_name'])
    fileCache[rec['file_name']] = {'file_code': rec['file_code']}
  fileDetails = getFilesPermissions(sqlCon, fileLst, aresObj.http['TEAM_DSC'])
  for file, details in fileDetails:
    nbFiles =  len(details)
    fileCache.update({'nbFiles': nbFiles, 'userDetails': details})
    summaryRecSet.append({'file': file, 'nbFiles': nbFiles, 'userDetails': details})

  summaryTable = aresObj.simpletable(summaryRecSet, [{'key': 'file', 'colName': 'Files'},
  ...
  aresObj.title("File Management - %s" % aresObj.http['REPORT_NAME'])
  aresObj.newline()
  aresObj.newline()
  createdTeams = []
